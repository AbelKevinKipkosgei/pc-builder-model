...
#     return (
#         False,
#         f"✗ CPU socket ({cpu['socket']}) does not match motherboard socket ({mobo['primary_socket']})",
#     )
...
#         return (
#             False,
#             "✗ Motherboard has an integrated CPU — no upgradable RAM slot info available",
#         )
#     if ram["ram_type"] == mobo["ram_type"]:
#         return True, f"✓ {ram['ram_type']} matches motherboard"
#     return (
#         False,
#         f"✗ RAM type ({ram['ram_type']}) does not match motherboard ({mobo['ram_type']})",
#     )
...
#         issues.append(
#             f"RAM capacity ({ram['capacity_gb']}GB) exceeds motherboard max ({mobo['max_memory']}GB)"
#         )
#     if ram["num_sticks"] > mobo["memory_slots"]:
#         issues.append(
#             f"RAM needs {ram['num_sticks']} slots, motherboard only has {mobo['memory_slots']}"
#         )
#     if issues:
...
#     return (
#         False,
#         f"✗ GPU length ({gpu['length']}mm) exceeds estimated case clearance ({case['estimated_max_gpu_length']}mm)",
#     )
...
#     case_ff = (
#         normalize_form_factor(case["form_factor_family"])
#         if pd.notnull(case["form_factor_family"])
#         else None
#     )

#     if (
#         case_ff is None
#         or mobo_ff not in FORM_FACTOR_RANK
#         or case_ff not in FORM_FACTOR_RANK
#     ):
#         return (
#             False,
#             f"✗ Could not determine form factor compatibility ({mobo['form_factor']} vs {case['type']})",
#         )
...
#     return (
#         False,
#         f"✗ Motherboard ({mobo['form_factor']}) is too large for case ({case['type']})",
#     )
...
#         return (
#             True,
#             f"✓ PSU ({psu['wattage']}W) covers estimated draw ({total_draw:.0f}W + {headroom_pct}% headroom = {required_wattage:.0f}W)",
#         )
#     return (
#         False,
#         f"✗ PSU ({psu['wattage']}W) insufficient — estimated draw needs {required_wattage:.0f}W ({total_draw:.0f}W + {headroom_pct}% headroom)",
#     )
...
#     ]
#     results = [
#         {"check": label, "passed": passed, "message": message}
...
# def score_gpu(row):
#     return row["memory"] * 10 + (
#         row["boost_clock"] if pd.notnull(row["boost_clock"]) else 0
#     )
...
#     filter_mask=None,
#     min_spend_pct=0.6,
#     predicted_penalty=0.95,
# ):
#     """
#     Pick the highest-scoring part among candidates that use at least min_spend_pct of the sub-budget.
#     Predicted-price parts get a small score penalty so real-priced parts win on close calls.
#     """
#     mask = (df_priced["price"] <= sub_budget) & (
#         df_priced["price"] >= sub_budget * min_spend_pct
#     )
...
#     if "price_is_predicted" in candidates.columns:
#         candidates["adjusted_score"] = candidates["score"] * candidates[
#             "price_is_predicted"
#         ].apply(lambda predicted: predicted_penalty if predicted else 1.0)
#     else:
#         candidates["adjusted_score"] = candidates["score"]

#     return candidates.sort_values("adjusted_score", ascending=False).iloc[0]
...
#     if cpu is None:
#         return {
#             "error": f"No CPU found within ${cpu_budget:.0f} budget allocation. Try increasing your total budget."
#         }
...
#         mobo_budget,
#         score_motherboard,
#         filter_mask=mobo_mask,
#         min_spend_pct=0.3,
#     )
#     if mobo is None:
#         return {
#             "error": f"Budget too tight: after CPU (${cpu['price']:.0f}), only ${mobo_budget:.0f} left for a {cpu['socket']} motherboard. Try a budget of at least ${total_budget + 100:.0f}."
#         }
...
#     if ram is None:
#         return {
#             "error": f"No RAM found matching {mobo['ram_type']} within budget allocation."
#         }
...
#     case = best_in_budget(
#         data["case_p"], case_budget, score_case, filter_mask=case_mask
#     )
#     if case is None:
#         return {
#             "error": "No case found with enough GPU clearance within budget allocation."
#         }
...
#     if psu is None:
#         return {
#             "error": f"No PSU found with {required_watts:.0f}W+ within budget allocation."
#         }
...
#     }
#     predicted_count = sum(
#         1 for p in parts.values() if p.get("price_is_predicted", False)
#     )
...
# def format_price(part):
#     """Format a part's price, marking it clearly if it's a model-estimated value rather than a real listed price."""
#     price = part["price"]
#     if pd.isnull(price):
#         return "Price unknown"
#     if part.get("price_is_predicted", False):
#         return f"~${price:.2f} (estimated)"
#     return f"${price:.2f}"
...
#         budget = st.slider(
#             "Budget ($)", min_value=400, max_value=4000, value=1200, step=50
#         )
...
#             total_label = f"${build['total_cost']:.2f}"
#             if build["predicted_count"] > 0:
#                 total_label += " *"
#             col1.metric("Estimated total", total_label)
#             col2.metric(
#                 "Compatibility",
#                 "All good ✓" if check["overall_compatible"] else "Issues found ✗",
#             )
#             col3.metric("Budget", f"${budget}")

#             if build["predicted_count"] > 0:
#                 st.caption(
#                     f"* {build['predicted_count']} of 6 parts have no listed price; "
#                     f"their cost is a model-based estimate, marked below."
#                 )
...
#                         "Component": "CPU",
#                         "Part": build["cpu"]["name"],
#                         "Price": format_price(build["cpu"]),
#                     },
#                     {
#                         "Component": "Motherboard",
#                         "Part": build["motherboard"]["name"],
#                         "Price": format_price(build["motherboard"]),
#                     },
#                     {
#                         "Component": "RAM",
#                         "Part": build["ram"]["name"],
#                         "Price": format_price(build["ram"]),
#                     },
#                     {
#                         "Component": "GPU",
#                         "Part": f"{build['gpu']['chipset']} — {build['gpu']['name']}",
#                         "Price": format_price(build["gpu"]),
#                     },
#                     {
#                         "Component": "Case",
#                         "Part": build["case"]["name"],
#                         "Price": format_price(build["case"]),
#                     },
#                     {
#                         "Component": "PSU",
#                         "Part": build["psu"]["name"],
#                         "Price": format_price(build["psu"]),
#                     },
#                 ]
#             )
...
#     else:
#         st.info(
#             "Set your budget and use case in the sidebar, then click **Generate build**."
#         )
...
#     st.caption(
#         "Each dropdown filters to only show options compatible with what you've already picked."
#     )
...
#     with col2:
#         mobo_candidates = data["mobo"][
#             data["mobo"]["primary_socket"] == cpu["socket"]
#         ].sort_values("name")
#         if mobo_candidates.empty:
#             st.warning(f"No motherboards found for socket {cpu['socket']}.")
#             mobo = None
#         else:
#             mobo_name = st.selectbox(
#                 f"Motherboard (socket: {cpu['socket']})",
#                 mobo_candidates["name"].tolist(),
#             )
...
#                 st.warning(
#                     f"No {mobo['ram_type']} kits found that fit this motherboard's limits."
#                 )
#                 ram = None
#             else:
#                 ram_name = st.selectbox(
#                     f"RAM (type: {mobo['ram_type']})", ram_candidates["name"].tolist()
#                 )
...
#     with col4:
#         gpu_options = (
#             data["gpu"].sort_values(["chipset", "name"]).reset_index(drop=True)
#         )
...
#                         and FORM_FACTOR_RANK[mobo_ff]
#                         <= FORM_FACTOR_RANK[normalize_form_factor(x)]
#                     )
#                 )
#             ].sort_values("name")
#             if case_candidates.empty:
#                 st.warning(
#                     "No cases found that fit both this GPU and this motherboard."
#                 )
#                 case = None
#             else:
...
#     with col6:
#         required_watts = (cpu["tdp"] + gpu["estimated_tdp"] + 100) * 1.2
#         psu_candidates = data["psu"][
#             data["psu"]["wattage"] >= required_watts
#         ].sort_values("name")
#         if psu_candidates.empty:
#             st.warning(f"No PSUs found with at least {required_watts:.0f}W.")
#             psu = None
#         else:
#             psu_name = st.selectbox(
#                 f"PSU (min: {required_watts:.0f}W)", psu_candidates["name"].tolist()
#             )
...
#             st.error(
#                 "Please resolve the warnings above before checking the full build."
#             )
#         else:
...
#             total_cost = sum(
#                 p["price"]
#                 for p in (cpu, mobo, ram, gpu, case, psu)
#                 if pd.notnull(p["price"])
#             )
#             missing_price = any(
#                 pd.isnull(p["price"]) for p in (cpu, mobo, ram, gpu, case, psu)
#             )
#             predicted_count = sum(
#                 1
#                 for p in (cpu, mobo, ram, gpu, case, psu)
#                 if p.get("price_is_predicted", False)
#             )

#             col1, col2 = st.columns(2)
#             total_label = f"${total_cost:.2f}"
#             if missing_price or predicted_count > 0:
#                 total_label += " *"
#             col1.metric("Estimated total", total_label)
#             col2.metric(
#                 "Compatibility",
...
#             if missing_price and predicted_count > 0:
#                 st.caption(
#                     f"* {predicted_count} part(s) have an estimated price; some others have no price data at all and are excluded from the total."
#                 )
#             elif predicted_count > 0:
#                 st.caption(
#                     f"* {predicted_count} part(s) have a model-estimated price rather than a real listed price, marked below."
#                 )
#             elif missing_price:
#                 st.caption(
#                     "* One or more selected parts has no listed price at all; total excludes them."
#                 )
...
#                         "Component": "CPU",
#                         "Part": cpu["name"],
#                         "Price": format_price(cpu),
#                     },
#                     {
#                         "Component": "Motherboard",
#                         "Part": mobo["name"],
#                         "Price": format_price(mobo),
#                     },
#                     {
#                         "Component": "RAM",
#                         "Part": ram["name"],
#                         "Price": format_price(ram),
#                     },
#                     {
#                         "Component": "GPU",
#                         "Part": f"{gpu['chipset']} — {gpu['name']}",
#                         "Price": format_price(gpu),
#                     },
#                     {
#                         "Component": "Case",
#                         "Part": case["name"],
#                         "Price": format_price(case),
#                     },
#                     {
#                         "Component": "PSU",
#                         "Part": psu["name"],
#                         "Price": format_price(psu),
#                     },
#                 ]
#             )
#             st.table(parts_table)
...
def dropdown_label(row):
    """Build a dropdown option label that includes price visibility, so users see cost before selecting."""
    price = row["price"]
    if pd.isnull(price):
        price_str = "price unknown"
    elif row.get("price_is_predicted", False):
        price_str = f"~${price:.2f} est."
    else:
        price_str = f"${price:.2f}"
    return f"{row['name']} — {price_str}"
...
# ---------------------------------------------------------------------------
    ...
    # --- CPU: no filtering, the starting point ---
    with col1:
        cpu_options = data["cpu"].sort_values("name").reset_index(drop=True)
        cpu_labels = cpu_options.apply(dropdown_label, axis=1)
        cpu_choice = st.selectbox("CPU", cpu_labels.tolist())
        cpu = cpu_options[cpu_labels == cpu_choice].iloc[0]

    # --- Motherboard: filtered to CPU's socket ---
    with col2:
        mobo_candidates = (
            data["mobo"][data["mobo"]["primary_socket"] == cpu["socket"]]
            .sort_values("name")
        ...
        if mobo_candidates.empty:
            ...
            mobo_labels = mobo_candidates.apply(dropdown_label, axis=1)
            mobo_choice = st.selectbox(
                f"Motherboard (socket: {cpu['socket']})", mobo_labels.tolist()
            )
            mobo = mobo_candidates[mobo_labels == mobo_choice].iloc[0]
            if pd.isnull(mobo["price"]):
                st.caption(
                    "⚠️ This board has no known or estimated price (often true for older/rare boards)."
                )
    ...
    with col3:
        if mobo is not None and pd.notnull(mobo["ram_type"]):
            ...
            if ram_candidates.empty:
                ...
                ram_labels = ram_candidates.apply(dropdown_label, axis=1)
                ram_choice = st.selectbox(
                    f"RAM (type: {mobo['ram_type']})", ram_labels.tolist()
                )
                ram = ram_candidates[ram_labels == ram_choice].iloc[0]
        else:
            st.warning("Select a compatible motherboard first.")
            ram = None
    ...
    with col4:
        ...
        gpu_labels = gpu_options.apply(
            lambda row: f"{row['chipset']} — {row['name']} — "
            + (
                "price unknown"
                if pd.isnull(row["price"])
                else (
                    f"~${row['price']:.2f} est."
                    if row.get("price_is_predicted", False)
                    else f"${row['price']:.2f}"
                )
            ),
            axis=1,
        )
        gpu_choice = st.selectbox("GPU", gpu_labels.tolist())
        gpu = gpu_options[gpu_labels == gpu_choice].iloc[0]
    ...
    with col5:
        if mobo is not None:
            ...
            if case_candidates.empty:
                ...
                case_labels = case_candidates.apply(dropdown_label, axis=1)
                case_choice = st.selectbox("Case", case_labels.tolist())
                case = case_candidates[case_labels == case_choice].iloc[0]
        else:
            st.warning("Select a compatible motherboard first.")
            case = None
    ...
    with col6:
        required_watts = (cpu["tdp"] + gpu["estimated_tdp"] + 100) * 1.2
        psu_candidates = (
            data["psu"][data["psu"]["wattage"] >= required_watts]
            .sort_values("name")
            .reset_index(drop=True)
        )
        if psu_candidates.empty:
            ...
            psu_labels = psu_candidates.apply(dropdown_label, axis=1)
            psu_choice = st.selectbox(
                f"PSU (min: {required_watts:.0f}W)", psu_labels.tolist()
            )
            psu = psu_candidates[psu_labels == psu_choice].iloc[0]
